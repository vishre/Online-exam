...
?>
<body style="background-image:url(image/q.jpg);background-repeat: no-repeat; background-attachment: fixed;background-size: cover;">
<div class="header">
<div class="top">
...
<!--quiz start-->
<?php
$a=mysqli_query($con,"SELECT eid FROM user_ans");
...
$w=mysqli_query($con,"SELECT *FROM user_ans WHERE eid='$eid' AND qid='$qid'");
while($row=mysqli_fetch_array($w) )
{
	$answer=$row['ansid'];
}
$e=mysqli_query($con,"SELECT *FROM answer WHERE qid='$qid'");
while($row=mysqli_fetch_array($e) )
{
	$crct=$row['ansid'];
}
$a=mysqli_query($con,"SELECT * FROM options WHERE qid='$qid' " );
...
if($optionid==$crct){
echo'<p style="color:green"><b>'.$option.'</b><span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></span></p><br />';
}

else if(($answer!=$crct) && ($answer==$optionid)){
echo'<p style="color:red">'.$option.'<span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></p><br />';
}
