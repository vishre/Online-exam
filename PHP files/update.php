...
if(@$_GET['fdid'] && $_SESSION['key']=='jungkook') {
$id=@$_GET['fdid'];
...
//delete user
if(isset($_SESSION['key'])){
if(@$_GET['demail'] && $_SESSION['key']=='jungkook') {
$demail=@$_GET['demail'];
...
//remove quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'rmquiz' && $_SESSION['key']=='jungkook') {
$eid=@$_GET['eid'];
...
//add quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='jungkook') {
...
$desc = $_POST['desc'];
$id=uniqid();
$q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$correct' , '$wrong','$total','$time' ,'$desc', NOW(),'$email1')");
...
//add question
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addqns' && $_SESSION['key']=='jungkook') {
$n=@$_GET['n'];
$eid=@$_GET['eid'];
$ch=@$_GET['ch'];
...
$ans=$_POST['ans'];
$qid=@$_GET['qid'];
$a=mysqli_query($con,"INSERT INTO user_ans VALUES('$email','$eid','$qid','$ans')")or die('error1');
...
if($sn == 1)
{
$q=mysqli_query($con,"INSERT INTO history VALUES('$email','$eid' ,'0','0','0','0',NOW())")or die('Error0');
}
...
$n=@$_GET['n'];
$t=@$_GET['t'];
$a=mysqli_query($con,"DELETE FROM `user_ans` WHERE eid='$eid' AND email='$email' " )or die('Error184');
$q=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND email='$email'" )or die('Error156');
while($row=mysqli_fetch_array($q) )
{
$s=$row['score'];
